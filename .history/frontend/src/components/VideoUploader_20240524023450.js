...
const VideoUploader = () => {
    ...
    const extractYouTubeTranscript = async (videoLink) => {
        try {
            ...
            const response = await axios.get(`http://localhost:6000/extract-transcript/${videoId}`);
    ...
    const generateSummary = async (transcript) => {
        try {
            const response = await axios.post('http://localhost:6000/generate-summary', { transcript });
