...
def save_summary_and_thumbnail(summary, thumbnail_url, video_id):
    # Find the directory path for my-appBackend
    current_directory = os.getcwd()
    my_app_backend_directory = None
    for root, dirs, files in os.walk(current_directory):
        if "my-appBackend" in dirs:
            my_app_backend_directory = os.path.join(root, "my-appBackend")
            break

    if my_app_backend_directory is None:
        st.error("my-appBackend directory not found!")
        return

    # Define the directory path for public/files
    directory_path = os.path.join(my_app_backend_directory, "public", "files")
    ...
    if thumbnail_url.startswith("http"):
        ...
        thumbnail_image_path = os.path.join(directory_path, f"{video_id}.jpg")
    ...
    summary_file_path = os.path.join(directory_path, f"{video_id}.txt")
    with open(summary_file_path, 'w') as f:
        f.write(summary)

    st.success(f"Summary and thumbnail saved in 'public/files' directory within my-appBackend with filename '{video_id}'")
