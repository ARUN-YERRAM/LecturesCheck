...
import summarize from 'summarize'; // Importing summarize directly as an ES6 module

// Function to fetch YouTube transcript data
const fetchTranscript = async (videoUrl) => {
    try {
        const videoId = videoUrl.split('=')[1];
        // Fetching data without using environment variables
        const response = await axios.get(`https://www.googleapis.com/youtube/v3/videos?part=snippet&id=${videoId}&key=AIzaSyBch2-7BysN3fGyXFpAnINIhWfoqUiVsoY`);
        const transcriptResponse = await axios.get(`https://www.googleapis.com/youtube/v3/captions?part=snippet&videoId=${videoId}&key=AIzaSyBch2-7BysN3fGyXFpAnINIhWfoqUiVsoY`);
...
const Analyze = () => {
    ...
    // AIzaSyBch2-7BysN3fGyXFpAnINIhWfoqUiVsoY
