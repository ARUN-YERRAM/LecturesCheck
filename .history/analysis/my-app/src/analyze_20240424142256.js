...
// Function to generate summary using Google Generative AI or alternative
const generateSummary = async (transcriptText, prompt) => {
    // Implement logic to use Generative AI or alternative if available
};
...
const YouTubeTranscriptConverter = () => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const summary = await generateSummary(transcriptText, "You are YouTube video summarizer. You will be taking the transcript text and summarizing the entire video and providing the important summary in points within 250 words. Please provide the summary of the text given here:");
            setSummary(summary);
        } catch (error) {
