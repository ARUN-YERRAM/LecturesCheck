...
(async () => {
    try {
        ...
        // Get all the video files in the files folder
        ...
        // If the folder files has no video file extensions, throw an error
        if (choices.length === 0) {
            console.log('No video files found in ./files folder. Please add some videos and try again.');
            return;
        }
        ...
        videoSelected.path = `./public/${videoSelected.filename}`;
        ...
        await fs.writeFile(`./files/${videoSelected.filename.replace(path.extname(videoSelected.filename), '')}_summary.txt`, summary);
        console.log(`Summary saved to ./files/${videoSelected.filename.replace(path.extname(videoSelected.filename), '')}_summary.txt`);
